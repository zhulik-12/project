...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ru: {
            ...
            aboutUs: {
                ...
                item3Title: 'Отечественный товар',
                item3Desc: 'Мы производим качественную продукцию в Казахстане',
            ...
            contact: {
                ...
                phone: 'Телефон:',
                phoneValue: '+7 701 531 58 72',
                email: 'Email:',
                instagram: 'Instagram:',
                instagramLink: '📷 @dala.dami.kz',
                name: 'Ваше имя',
        ...
        kz: {
            ...
            aboutUs: {
                ...
                item3Title: 'Отандық тауар',
                item3Desc: 'Біз Қазақстанда сапалы өнімдер өндіреміз',
    ...
    function updateLanguage(lang) {
        ...
        // Анимация перехода для текста
        const textElements = document.querySelectorAll('.nav-menu a, .hero-title, .hero-subtitle, .hero-description, .section-title, h1, h2, h3, p, span, .product-card h3, .product-card p, .benefit-item h3, .benefit-item p, .about-us-item h3, .about-us-item p, .feature h3, .feature p');
        ...
        if (orderBtn) {
            orderBtn.textContent = t.hero.order;
            // Убеждаемся, что ссылка на WhatsApp правильная
            if (orderBtn.tagName === 'A') {
                orderBtn.href = 'https://wa.me/77015315872';
        ...
        if (contactSection) {
            ...
            if (contactItems[1]) contactItems[1].innerHTML = `<strong>${t.contact.email}</strong> ErkinGaisin999@gmail.com`;
            if (contactItems[2]) contactItems[2].innerHTML = `<strong>${t.contact.instagram}</strong> <a href="https://www.instagram.com/dala.dami.kz?igsh=MWszbWFybHRvaWg0ZA==" target="_blank" style="color: #FFD700; text-decoration: none; margin-left: 5px;">${t.contact.instagramLink}</a>`;
        ...
        footerTexts[1].textContent = t.footer.tagline;
            ...
            // Возвращаем видимость с анимацией
            setTimeout(() => {
                textElements.forEach(el => {
                    el.style.opacity = '1';
...
function checkMobileView() {
    isMobileView = document.body.classList.contains('mobile-view');
}

checkMobileView();

// Наблюдаем за изменениями класса mobile-view
const mobileViewObserver = new MutationObserver(() => {
    checkMobileView();
});

mobileViewObserver.observe(document.body, {
    attributes: true,
    attributeFilter: ['class']
});
...
window.addEventListener('scroll', () => {
    ...
    // Обновляем состояние кнопки скрытия header
    if (scrollToTopBtn) {
        if (header.classList.contains('hidden')) {
            scrollToTopBtn.classList.add('show-header');
            scrollToTopBtn.classList.remove('hide-header');
            scrollToTopBtn.textContent = '↓';
        } else {
            scrollToTopBtn.classList.add('hide-header');
            scrollToTopBtn.classList.remove('show-header');
            scrollToTopBtn.textContent = '↑';
        }
    }
    ...
    // Для мобильного режима: header скрывается и не появляется автоматически
    if (isMobileView) {
        // Если прокрутка вниз и больше 50px - скрываем
        if (currentScroll > lastScrollTop && currentScroll > 50) {
            header.classList.add('hidden');
        }
        // В мобильном режиме header не появляется автоматически при прокрутке вверх
        // Только в самом верху
        if (currentScroll <= 0) {
            header.classList.remove('hidden');
        }
    } else {
        // Для ПК: обычная логика
        if (currentScroll > lastScrollTop && currentScroll > 50) {
            header.classList.add('hidden');
...
// Переключение между ПК и мобильной версией
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedDevice === 'mobile') {
        ...
        deviceButtons[0].classList.remove('active');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Список возможных файлов изображений (включая новые скриншоты)
    const imageFiles = [
        'Снимок экрана 2025-12-30 173844.png',
        'Снимок экрана 2025-12-27 183236.png',
        'сертификат.jpg', 
        'сертификат.png', 
        'certificate.jpg', 
        'certificate.png', 
        'Индустриальный сертификат.jpg', 
        'Индустриальный сертификат.png'
    ];
